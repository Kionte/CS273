	...
	if (column == NUMBER_COLS) { // Base Condition
		...
		for (int row = 0; row < RowPlacement.size(); ++row) {
			...
			if (PlaceQueen(column + 1)) {
				return true; // the only time it will reach true is when it reaches the end of the colomns. once it reaches col # 7 (the end) 
				// it will ogo to the if statment at the top of the function. becasue col will = numOfCol it will return but this time it will not recursily go back into place queen. which means that fucntion will return true and will go back to the next instance of itself and return true and repeat itself until it reaches the original 
			}
				// FIXME: Unassign the assignment of the column's queen to this row, and try the next one
			//std::cout << RowPlacement[row] <<  " couting the qween in colomn: " << column << std::endl;
			RowPlacement[row] = UNASSIGN_VALUE;
				n++;
			//std::cout << RowPlacement[row] << " couting the qween in colomn: post delete " << column << std::endl;
