...
bool PlaceQueen(int column) {
	...
	if (column == NUMBER_COLS) { // Base Condition
		PlacementSolutions.push_back(RowPlacement); // add solution to placement solution 

		if (PlaceQueen(column - 1)) { // backtracking with in backtracking used to find every unique itterations
			PlaceQueen(column + 1); // doesnt make any difference who knew
			return true;
		}
	}
	else {
		...
		for (int row = 0; row < RowPlacement.size(); ++row) {
			...
			if (RowPlacement[row] != UNASSIGN_VALUE) // check if spot is availible
				continue;
			if (!ValidPosition(column, row)) // check if it brreaks any of the rules
				continue;
...
int main()
{
	...
	cout << "Number of backtracks: " << n << endl << endl;
	;
}